...
class SimpleChatbotEvaluator:
    ...
    def create_test_samples(self, test_split: float = 0.2) -> None:
        ...
        for intent in self.chatbot.intents["intents"]:
            ...
            for pattern in test_patterns:
                self.test_samples.append({"text": pattern, "expected_tag": tag})
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        for sample in self.test_samples:
            ...
            predicted_tag = None
            confidence = (
                0.0  # Placeholder since we don't have confidence from predict()
            )
        ...
        if incorrect:
            ...
            for i, result in enumerate(incorrect[:5]):  # Show up to 5 examples
                ...
                print(
                    f"Predicted: {result['predicted']} (Confidence: {result['confidence']:.2f})"
                )
